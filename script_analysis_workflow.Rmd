...
In total, there are 58 proteins for which we do find interaction effects. We only show the 55 proteins with an $R^2>.2$ in the visualizations.
...
# pdf("heatmap-L.pdf", width = 53*2/6, height = 33/2)
pheatL <- pheatmap(L, cluster_cols = F, cluster_rows = F,
         color = c("grey96", "grey52"), #cutree_rows = 53,
         fontsize_row = 7, fontsize_col = 15,
         cellheight = 16, cellwidth = 16, border_color = "white",
          angle_col = 90,
         #gaps_col = 1:12, gaps_row = 1:33
         )
# dev.off()
```
...
```{r eval = F, echo = F}
# pdf("plots_y_yhat_RNF2.pdf",
#     width = 6/1.3, height = 4.4/1.3)
# plot_list_p_phat[which(Proteins_int == "RNF2")]
# dev.off()
# pdf("/Users/mara.stadler/LRZ Sync+Share/PhD/MUDS_Projekt/temp/plots_y_yhat.pdf",
#     width = 22/2, height = 29/1.8)
# 
# grid.arrange(grobs = plot_list_p_phat[1:(21)],
#              ncol = 3, nrow = 7)
# grid.arrange(grobs = plot_list_p_phat[22:(42)],
#              ncol = 3, nrow = 7)
# grid.arrange(grobs = plot_list_p_phat[43:(58)],
#              ncol = 3, nrow = 7)
# 
# dev.off()
```
...
```{r echo = F, eval = F}
#names(r_squared_all) <- colnames(coef_matrix)
#ddf <-as.data.frame(sort(r_squared_all, decreasing = T))
```
...
```{r echo = F, eval =T}
create_stability_plots <- function(protein_set = Proteins_int){
  stab_plot_list <- list()
  n <- 0
  for(pint in protein_set){

        p <- which(colnames(P) == paste0(pint, "_F"))
    if(class(CPSS_F[[p]]) == "empty" | class(CPSS_R[[p]]) == "empty"){
      next
    }
    pi_F <- sort(CPSS_F[[p]]$max, decreasing = T)  
    if(is.null(pi_F)){
      next
      }
    names(pi_F) <- gsub('H2A.Z H2A.Z','H2A.Z', names(pi_F))
    dfF_start <- data.frame("Mod" = names(pi_F), "Pi_max" = pi_F)
    dfF_start$Mod <- factor(dfF_start$Mod, 
                            levels = dfF_start$Mod)
    
   
    pi_R <- sort(CPSS_R[[p]]$max, decreasing = T)  
     if(is.null(pi_R)){
      next
      }
    names(pi_R) <- gsub('H2A.Z H2A.Z','H2A.Z', names(pi_R))
    dfR <- data.frame("Mod" = names(pi_R), "Pi_max" = pi_R)
    ## same order as Forward
    dfR$Mod <- factor(dfR$Mod, 
                      levels = dfF_start$Mod)
    
    modF <- dfF_start$Mod[which(dfF_start$Pi_max > 0)]
    modR <- dfR$Mod[which(dfR$Pi_max > 0)]
    modFR <- base::union(modF, modR)
    dfF <- dfF_start[which(dfF_start$Mod %in% modFR), ]
    
    dfR <- dfR[which(dfR$Mod %in% modFR), ]
    

    n <- n + 1
    #dfF$Mod <- factor(dfF$Mod, levels = dfF$Mod)
    stab_plot_list[[n]] <- ggplot(data = dfF, aes(x = Mod, y = Pi_max)) +
      geom_bar(stat="identity", color="gray80", fill="gray80") + ylim(0,1) + coord_flip() + 
      theme_minimal() +
      scale_color_manual(values=c("white")) +
      geom_hline(yintercept = .5, color = "gray") +
      theme(axis.text.x = element_text(family = "Helvetica", colour="black", size = 13),
            axis.title.x = element_text(family = "Helvetica", colour="black", size=18),
          axis.text.y = element_text(family = "Helvetica", colour="black", size=13),
          axis.title.y = element_text(family = "Helvetica", colour="black", size=0),
          plot.title = element_text(family = "Helvetica", colour="black", size=23)) +
      ggtitle(paste(pint, "- Forward")) + 
      ylab(bquote(hat(pi))) + 
      xlab("Feature")  
    ...
    n <- n + 1
    stab_plot_list[[n]] <- ggplot(data = dfR, aes(x = Mod, y = Pi_max)) +
      geom_bar(stat="identity", color="gray80", fill="gray80") + ylim(0,1) + coord_flip() + 
      theme_minimal() +
      geom_hline(yintercept = .5, color = "gray") +
      theme(axis.text.x = element_text(family = "Helvetica", colour="black", size=13),
          ...
          axis.title.y = element_text(family = "Helvetica", colour="black", size=0),
          plot.title = element_text(family = "Helvetica", colour="black", size=23)) +
      ggtitle(paste(pint, "- Reverse")) + 
      ylab(bquote(hat(pi))) + 
      xlab("Feature") 
  }
  
  return(stab_plot_list)
}

plt_CBX8 <- create_stability_plots("CBX8")
```


```{r echo=F, eval=F}
# ggsave("stability_plots_CBX8F.pdf", plot = plt_CBX8[[1]], width = 5, height = 9, dpi = 600, device = "pdf")
# ggsave("stability_plots_CBX8R.pdf", plot = plt_CBX8[[2]], width = 5, height = 9, dpi = 600, device = "pdf")

# stabPlots_all_proteins <- create_stability_plots(colnames(coef_matrix))
# 
# pdf("stability_bar_plots_all_proteins.pdf",
#     width = 30, height = 45)
# i <- 0
# while(i <= 1915 * 2){
#   i <- i + 12
#   grid.arrange(grobs = stabPlots_all_proteins[(i-11):i],
#                ncol = 4, nrow = 3)
# }
# 
# dev.off()
